...
class QtyState(StatesGroup):
    """FSM for changing the asset quantity and buy price in the portfolio."""
...
def _pf_quote_kb(asset_type: str, symbol: str) -> InlineKeyboardMarkup:
    ...
    if asset_type == "stock":
        ...
        row1.append(
            InlineKeyboardButton(
                text=t("menu.btn.chart"), callback_data=f"stock_chart:{symbol}"
            )
        )
...
async def on_pf_qty(callback: CallbackQuery, state: FSMContext) -> None:
    """Starts the FSM for changing the asset quantity (and buy price)."""
    symbol = callback.data.split(":", 2)[2]
...
async def on_qty_value(message: Message, state: FSMContext) -> None:
    ...
    builder.row(
        InlineKeyboardButton(
            text=t("portfolio.btn.skip_price"), callback_data="pf:qty_skip_price"
        ),
        InlineKeyboardButton(
            text=t("portfolio.btn.cancel"), callback_data="pf:qty_cancel"
        ),
...
@router.message(QtyState.buy_price)
...
async def on_qty_skip_price(
    callback: CallbackQuery, state: FSMContext, cache: TTLCache
) -> None:
    """Saves the quantity, keeps the current buy price."""
    data = await state.get_data()
    symbol = data.get("symbol")
    qty: float | None = data.get("quantity")
    if not symbol or qty is None:
        ...
        await callback.answer(t("portfolio.stale"), show_alert=True)
        return
    await _update_qty_price(callback.from_user.id, symbol, qty, None)
    await state.clear()
    _, kb = await _render_pf_menu(callback.from_user.id, cache)
    await callback.message.answer(
        t("portfolio.qty.saved", symbol=symbol, qty=_fmt_qty(qty)),
...
async def _update_qty_price(
    telegram_id: int, symbol: str, quantity: float | None, buy_price: float | None
) -> None:
    """Updates the asset quantity and optionally the buy price in the DB."""
    values: dict[str, object] = {"quantity": quantity}
    if buy_price is not None:
